...
Endpoints:
  ...
  GET  /document-types/{dt_id}/linked-sources
  POST /document-types/{dt_id}/linked-sources
  PUT  /document-types/{dt_id}/linked-sources/{src_id}
  DELETE /document-types/{dt_id}/linked-sources/{src_id}
  POST /document-types/{dt_id}/linked-sources/{src_id}/invoke
"""
...
from app.models.doc_type_settings import DocTypeSapConfig, DocTypeApiSource, DocTypeLinkedSource
...
def _do_http_invoke(
    ...
    """Shared HTTP-call logic for both API sources and linked sources."""
    params: dict[str, str] = {}
    if select_fields and select_fields.strip():
        params["$select"] = select_fields.strip()
    if filter_template and filter_template.strip():
        f = filter_template.strip()
        for k, v in context.items():
            if v is not None:
                f = f.replace(f"{{{k}}}", str(v))
        params["$filter"] = f
    if extra_params and extra_params.strip():
        for part in extra_params.strip().split("&"):
            part = part.strip()
            if "=" in part:
                k2, v2 = part.split("=", 1)
    ...
    if resp.status_code == 401 and use_sap_auth:
        cfg = _get_or_create_sap_cfg(db, dt_id)
        _dt_sap_session_cache.pop(_sap_cache_key(cfg), None)
...
# ─── Linked Source Pydantic schemas ──────────────────────────────────────────
...
# ─── Linked Source serializer ─────────────────────────────────────────────────

def _serialize_linked_source(src: DocTypeLinkedSource) -> dict:
    def _parse(col) -> list:
        if not col:
            return []
        try:
...
@router.get("/{dt_id}/linked-sources", summary="Danh sách chứng từ liên kết của loại chứng từ")
async def list_linked_sources(
    dt_id: int,
    db: Session = Depends(get_db),
    _: CurrentUser = Depends(get_current_user),
):
    sources = db.query(DocTypeLinkedSource).filter(
        DocTypeLinkedSource.document_type_id == dt_id
    ).order_by(DocTypeLinkedSource.name).all()
    return [_serialize_linked_source(s) for s in sources]


@router.post("/{dt_id}/linked-sources", summary="Tạo chứng từ liên kết cho loại chứng từ", status_code=201)
...
@router.put("/{dt_id}/linked-sources/{src_id}", summary="Cập nhật chứng từ liên kết")
async def update_linked_source(
    ...
    src = db.get(DocTypeLinkedSource, src_id)
    if not src or src.document_type_id != dt_id:
...
@router.delete("/{dt_id}/linked-sources/{src_id}", summary="Xoá chứng từ liên kết")
...
@router.post("/{dt_id}/linked-sources/{src_id}/invoke", summary="Gọi chứng từ liên kết")
async def invoke_linked_source(
    ...
    logger.info("Invoking linked source '%s' (dt=%d): GET %s", src.name, dt_id, src.base_url)
    return _do_http_invoke(
        base_url        = src.base_url,
        select_fields   = src.select_fields,
        filter_template = src.filter_template,
        extra_params    = src.extra_params,
